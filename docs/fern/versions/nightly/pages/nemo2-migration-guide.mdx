...
For fine-tuning, use `bridge.training.config.GPTSFTDatasetConfig` for local JSONL or Hugging Face maker data and set `checkpoint.pretrained_checkpoint` to the base model. Optionally add a `peft` configuration for parameter-efficient training.
...
NeMo 2.0 uses `PreTrainingDataModule` and `FineTuningDataModule` classes. Megatron Bridge uses `bridge.training.config.GPTDatasetConfig` for pretraining and `bridge.training.config.GPTSFTDatasetConfig` for text fine-tuning.
