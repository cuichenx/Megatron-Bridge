...
| Full SFT | `GPTSFTDatasetConfig` for local JSONL or Hugging Face maker data | `finetune()` | Use `checkpoint.pretrained_checkpoint` for the base model, or `checkpoint.load` for a full native Megatron resume |
